...
from qiskit_ibm_runtime.executor.routines.estimator_v2.estimator_v2 import EstimatorV2
...
class TestEstimatorV2Run(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.executor_patcher = patch(
            "qiskit_ibm_runtime.executor.routines.estimator_v2.estimator_v2.Executor"
        )
    ...
    def test_run_with_custom_prepare_function(self):
        """Test run with custom prepare function."""
        custom_prepare_called = []

        def custom_prepare(pubs, options, precision):
            custom_prepare_called.append(True)
            # Create a minimal quantum program
            shots = int(math.ceil(1.0 / (precision**2))) if precision else 1024
            qp = QuantumProgram(items=[], shots=shots)
            return qp, ExecutorOptions()

        estimator = EstimatorV2(mode=self.backend, custom_prepare=custom_prepare)
        ...
        # Verify custom prepare was called
